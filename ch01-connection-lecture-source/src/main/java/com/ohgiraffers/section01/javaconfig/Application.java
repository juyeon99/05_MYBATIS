...
public class Application {
    ...
    * MyBatis Mapper 사용방식
    * 1. 자바 인터페이스 파일 사용
    * -> 추상 메소드에 Annotation으로 쿼리 입력하고, getMapper로 쿼리를 불러와 실행
    ...
    * -> 직접 SQL 구문을 작성하기 때문에 복잡한 쿼리 작성할 때 용이
    ...
    * */
    ...
    public static void main(String[] args) {
        ...
        * build(): 설정에 대한 정보를 담고있는 Configuration 타입 객체를 받아 SqlSessionFactory 객체를 반환하는 메소드
        * */
        ...
        *                boolean 타입을 인자로 전달
        * - false: Connection 인터페이스 타입 객체로 DML 수행 후 Auto commit에 대한 옵션을 false로 지정
        * - true:  Connection 인터페이스 타입 객체로 DML 수행 후 Auto commit에 대한 옵션을 true로 지정
        * */
        SqlSession sqlSession = sqlSessionFactory.openSession(false);

        // getMapper(): configuration에 등록된 mapper를 동일 타입에 대해 반환
